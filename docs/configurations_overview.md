...
Field Operations are created based on occurrence information in the same field, dates intervals and crop. Thus,
if this config is set to `true`, files will be filtered by provider and type of operation and thus merged into different Field Operations. The default value is `false`.
