...
### Status
&nbsp<span class="badge badge--success">GET</span> `/users/{leafUserId}/cnhi-credentials/status`

Get the CNHI credentials status of the Leaf User based on its id and returns a JSON containing the status.
...
<Tabs
  ...
  const endpoint = 'https://api.withleaf.io/services/usermanagement/api/users/{leafUserId}/cnhi-credentials/status'
  const headers = { 'Authorization': `Bearer ${TOKEN}` }
  ...
  endpoint = 'https://api.withleaf.io/services/usermanagement/api/users/{leafUserId}/cnhi-credentials/status'
  headers = {'Authorization': f'Bearer {TOKEN}'}
