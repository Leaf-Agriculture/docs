...
Get the Sentera credentials status of the Leaf User based on its id and returns a JSON containing the status.
...
<Tabs
  ...
  const endpoint = 'https://api.withleaf.io/services/usermanagement/api/users/{leafUserId}/sentera-credentials/status'
  const headers = { 'Authorization': `Bearer ${TOKEN}` }
  ...
  endpoint = 'https://api.withleaf.io/services/usermanagement/api/users/{leafUserId}/sentera-credentials/status'
  headers = {'Authorization': f'Bearer {TOKEN}'}
