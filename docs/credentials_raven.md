...
### Status
&nbsp<span class="badge badge--success">GET</span> `/users/{leafUserId}/raven-credentials/status`

Get the Raven credentials status of the Leaf User based on its id and returns a JSON containing the status.
...
<Tabs
  ...
  const endpoint = 'https://api.withleaf.io/services/usermanagement/api/users/{leafUserId}/raven-credentials/status'
  const headers = { 'Authorization': `Bearer ${TOKEN}` }
  ...
  endpoint = 'https://api.withleaf.io/services/usermanagement/api/users/{leafUserId}/raven-credentials/status'
  headers = {'Authorization': f'Bearer {TOKEN}'}
